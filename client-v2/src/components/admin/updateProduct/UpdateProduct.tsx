...
const UpdateProduct = () => {
  ...
  const mutation = useMutation(
    (dataUpdate: any) =>
      updateProduct(aixosPrivate, data?.product._id as string, dataUpdate),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(["detail-product", router.query.slug]);
      },
    }
  );
      ...
      <div className={cx("wrapper")}>
        <Formik<CreateProduct>
          initialValues={{
            ...
            categories: data?.product.categories.map((i) => i._id) as string[],
          }}
            ...
            try {
              if (values.images.length > 0) {
                ...
                if (uploadImgRes.data.success) {
                  values.images = uploadImgRes.data.images.map(
                    (img) =>
                          ...
                          ? values.specialField.map((item, index) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  {Object.keys(filter.filter.specialField).map(
                                    (i, index) => (
...
const DescriptionProduct = ({
  ...
  const mutation = useMutation(() =>
    createOrUpdateDescription(aixosPrivate, id, { product: id, desc: rawHtml })
  );
