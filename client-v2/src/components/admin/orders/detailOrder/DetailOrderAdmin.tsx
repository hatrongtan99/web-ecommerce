...
const DetailOrderAdmin = () => {
  ...
  const mutation = useMutation(
    (data: ChangeStatusOrder) =>
      changeStatusNoteOrder(axiosPrivate, detailOrder?.data.order._id!, data),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(["detail-order", router.query.idOrder]);
      },
    }
  );
        ...
        <div className={cx("detail-top")}>
          <div className={cx("detail-top__left")}>
            <p>
              ...
              />{" "}
              {dFormat(new Date(detailOrder?.data.order.created!))}
            </p>
          ...
          <div className={cx("detail-top__right")}>
            <select
              ...
              id={cx("status-order")}
              onChange={(e) =>
                setDataStatus({ ...dataStatus, status: e.target.value })
              }
            ...
            >
              {optionsStatusOrders.map((option, index) => {
                if (option.value === dataStatus.status) {
                  return (
                    <option value={dataStatus.status} key={index}>
                      {option.label}
                    </option>
                  );
                } else {
            ...
            <span className={cx("print-icon")} onClick={handlePrint}>
        ...
        <div className={cx("detail-body")}>
          <div className={cx("detail-body__info")}>
            ...
            <div className={cx("admin-note")}>
              <textarea
                ...
                onChange={(e) =>
                  setDataStatus({ ...dataStatus, noteByAdmin: e.target.value })
                }
              />
              <Button
                size="sm"
                ...
                onClick={() =>
                  handleSubmit({ noteByAdmin: dataStatus.noteByAdmin })
                }
              >
                Save note
              </Button>
          ...
          <div className={cx("detail-body__product")}>
            <div className={cx("table-header")}>
              ...
              <p style={{ width: "20%", textAlign: "center" }}>Số lượng</p>
              <p style={{ width: "10%" }}>Giá mua</p>
              <p style={{ width: "10%", textAlign: "end" }}>Thành tiền</p>
            </div>
            {detailOrder?.data.order.cart.products.map((product) => {
              return (
                <div className={cx("table-item")} key={product.product._id}>
                  <div
                    style={{
                      width: "60%",
                    }}
          ...
          <div className={cx("detail-body__footer")}>
            <p>
              Tổng cộng:{" "}
              <span>
                {detailOrder?.data.order.totalPrice.toLocaleString()}đ
              </span>
            </p>
          </div>
        </div>
