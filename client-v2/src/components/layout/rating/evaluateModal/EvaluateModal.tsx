...
const EvaluateModal = ({ title, id }: EvaluateModalProps) => {
  ...
  const handleSubmitForm = async (e: Event) => {
    e.preventDefault();
    try {
      const res = await mutation.mutateAsync({ ...info, rating: evlStar });
      if (res.success) {
    ...
    <div className={cx("modal")} id="modal-evalu">
      ...
      <form className={cx("modal__box")}>
            ...
            {new Array(5).fill(0).map((star, index) => (
              ...
              >
                <AiFillStar
                  key={index}
                  size={40}
                  color={index + 1 <= evlStar ? "#f4c91f" : "#ddd"}
                  style={{
                    stroke: index + 1 <= evlStar ? "#f6ab27" : "#ddd",
                    strokeWidth: "70",
                  }}
                />
                <p>{evlString[index]}</p>
        ...
        <div className={cx("modal__box__footer")}>
          ...
          <p>
            Hệ thống sẽ xét duyệt đánh giá của bạn trước khi hiển thị lên
            website.
          </p>
        </div>
      </form>
