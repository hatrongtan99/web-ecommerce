...
const DetailOrder = ({ order }: { order: DetailOrderUser }) => {
  ...
  const option = useMemo(() => {
    return optionsStatusOrders.find((option) => option.value === order.status);
  }, [order]);

  const mutation = useMutation(
    () =>
      changeStatusNoteOrder(axiosPrivate, order._id, { status: "cancelled" }),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(["order-user", router.query.idOrder]);
      },
    }
  );
  ...
  return mutation.isLoading ? (
    ...
    <div className={`my-4 ${cx("container")}`}>
      <div className={cx("order-top")}>
        ...
        <div className={cx("order-top__info")}>
          ...
          <p>
            MÃ ĐƠN HÀNG:{" "}
            <span style={{ color: "#333" }}>#{order._id.slice(-7)}</span>
          </p>
        </div>
      </div>
      ...
      <div className={cx("detail-body")}>
        <div className={cx("detail-body__info")}>
          ...
          <div className={cx("user-note")}>
            <textarea placeholder="Ghi chú" value={order.note} readOnly />
          </div>
        </div>
        ...
        <div className={cx("detail-body__product")}>
          <div className={cx("table-header")}>
            ...
            <p style={{ width: "15%", textAlign: "center" }}>Số lượng</p>
            <p style={{ width: "15%" }}>Giá mua</p>
            <p style={{ width: "15%", textAlign: "end" }}>Thành tiền</p>
          </div>
        ...
        <div className={cx("detail-body__footer")}>
          <p>
            Tổng cộng: <span>{order.totalPrice.toLocaleString()}đ</span>
          </p>
        </div>
      </div>
      <BackdropModal
