...
class Order {
    ...
    async #getSession(userId) {
        return await db.execute(
            'SELECT Order_session_ID AS id FROM order_session WHERE User_ID = ?',
            [userId]
        );
    }
    ...
    async #getExitsProductInCart(productId, orderSessionId) {
        return await db.execute(
            'SELECT Cart_item_ID AS cartItemId, quantity FROM cart_item WHERE Product_ID = ? AND Order_session_ID = ?;',
            [productId, orderSessionId]
        );
    }
    ...
    async orderProducts(req) {
        ...
        const { userName, typeSex, userEmail, userPhone, userAddress, note } =
            req.body;
    ...
    async addToCart(req) {
        ...
        if (getSession.length == 0) {
            const res = await db.execute(
                'INSERT INTO order_session (User_ID) VALUES (?)',
                [userId]
            );
            const id = res.insertId;
            const insertToCart = await db.execute(
                'INSERT INTO cart_item (Product_ID, Order_session_ID, Quantity) VALUES (?, ?, ?);',
                [productId, id, quantity]
            );
            return insertToCart?.affectedRows;
        } else {
            const id = getSession[0].id;
            const exitsProduct = await this.#getExitsProductInCart(
                productId,
                id
            );
            if (exitsProduct.length == 0) {
                ...
                const updateQuantity = await db.execute(
                    'UPDATE cart_item set quantity = ? WHERE Product_ID = ? AND Order_session_ID = ?;',
                    [newQuantity, productId, id]
                );
