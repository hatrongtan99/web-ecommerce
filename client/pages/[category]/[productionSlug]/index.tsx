...
import type {
    ProductBycategoryAndSlugResult,
...
const ProductionSlug: NextPage<ProductionProps> = ({ data }) => {
    const id = useAppSelector((state) => state.checkout.userSessionId);
    return (
        <main className="main-content">
            <div className="container">
                <Breadcrumb />
                <Link href={`/checkout/6757353120781660100826405`}>click</Link>

                {/* <TitleDeltailProduct title={data.productName}/> */}
...
export const getStaticPaths: GetStaticPaths = async () => {
    ...
    Promise.all(
        categories.data.map((categoryResponse: CategoryResponse) =>
            productionApi.getProductsByCategory(
                categoryResponse.categorySlug,
                '1'
            )
        )
    ).then((results) => {
        results.forEach((result) => {
            ...
            paths.push({
                params: {
                    category: firtProductByCategory?.products[0].categorySlug,
                    productionSlug: firtProductByCategory?.products[0].slug,
                },
            });
        });
    });
...
export const getStaticProps: GetStaticProps<ProductionProps> =
    wrapper.getStaticProps((store) => async ({ params }) => {
        const { category, productionSlug } = params!;
        try {
            const productBycategoryAndSlug =
                await productionApi.getProductBycategoryAndSlug(
                    category as string,
                    productionSlug as string
                );
            store.dispatch(
                saveProductByCategoryAndSlug(productBycategoryAndSlug.data)
            );
            return {
                props: {
