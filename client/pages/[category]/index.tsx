...
export const getServerSideProps: GetServerSideProps<CategoryProps> =
    wrapper.getServerSideProps((store) => async ({ params, query }) => {
        let { page, category, ...rest } = query;
        try {
            const getProductsByCategory = productionApi.getProductsByCategory(
                category as string,
                page as string,
                rest
            );
            const getAllBrands = productionApi.getAllBrands();
            const [dataProductByCategory, dataAllBrands] = await Promise.all([
